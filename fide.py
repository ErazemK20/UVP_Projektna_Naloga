...
ID1 = 14625500 # Zaprt interval  obravnavanih
...
for i in range(ID1, ID2 + 1):
   ...
   if not m: # vsak element iz [ID1,ID2] ni identifikacijska stevilka
      ...
      if(drzava == "Slovenia" and standardni != "" and pospeseni != "" and hitropotezni != ""): # Podseznam za slovenskega sahista
...
standard_povprecje = 0   # Za dolocanje povprecnega standardnega ranga
pospeseni_povprecje = 0   # Za dolocanje povprecnega pospesenega ranga
hitropotezni_povprecje = 0    # Za dolocanje povprecnega hitropoteznega ranga

f = open(IZHODNA_DATOTEKA, 'w') # Odpri izhodno datoteko za izpis
for sahist in urejen_seznam:
   print (sahist[7], sahist[8], sahist[9], sahist[2], sahist[3], ",", "roj.", sahist[4], ",", sahist[6])
   standard_povprecje += int(sahist[7])
   pospeseni_povprecje += int(sahist[8])
   hitropotezni_povprecje += int(sahist[9])
   vrstica = str(sahist[7]) + " " + str(sahist[8]) + " " + str(sahist[9]) + " " + str(sahist[2]) + " " + str(sahist[3]) + " " + str(sahist[4]) + " " + str(sahist[6] + "\n")
   f.write(vrstica)

f.close()

standard_povprecje /= len(urejen_seznam)
pospeseni_povprecje /= len(urejen_seznam)
hitropotezni_povprecje /= len(urejen_seznam)

print ("Povprecen standardni rang: ", int(standard_povprecje))
print ("Povprecen pospeseni rang: ", int(pospeseni_povprecje))
print ("Povprecen hitropotezni rang: ", int(hitropotezni_povprecje))
